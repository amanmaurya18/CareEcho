...
export const CurrentDoseBanner: React.FC = () => {
  const { nextMedication, markAsTaken, language, patientName, voiceGender, testTriggerAlarm } = useCare();
  ...
  return (
    <section aria-label="Current Dose Hero" className="w-full my-6">
      <div className={`relative overflow-hidden rounded-3xl border-2 transition-all shadow-sm ${
        isTaken
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className={`inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-sm sm:text-base font-bold uppercase tracking-wider ${
              isTaken
              ...
              <span>{isTaken ? 'Completed' : isOverdue ? `Time is Over (${nextMedication.scheduledTime})` : `Due at ${nextMedication.scheduledTime}`}</span>
            </span>
          ...
          <div className="flex items-center gap-2">
            ...
            {!isTaken && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-red-300 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold transition-colors shadow-xs"
              >
            ...
            <span className="text-sm font-semibold text-slate-500">
              {nextMedication.timeOfDay} Schedule
            </span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-4 mb-4">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-sage-100 border border-sage-200 flex items-center justify-center shrink-0 mt-1 text-sage-700">
            {nextMedication.form === 'liquid' || nextMedication.form === 'drops' ? (
              <Droplets className="w-8 h-8 sm:w-9 sm:h-9" />
            ) : nextMedication.form === 'inhaler' ? (
              <Wind className="w-8 h-8 sm:w-9 sm:h-9" />
            ) : nextMedication.form === 'injection' ? (
              <Syringe className="w-8 h-8 sm:w-9 sm:h-9" />
            ) : (
          ...
          <div className="flex-1">
            <h2 className="text-2xl sm:text-4xl font-extrabold text-slate-900 tracking-tight leading-tight">
              {nextMedication.scheduledTime} — {nextMedication.name}{' '}
              <span className="text-sage-700 font-bold">{nextMedication.dosage}</span>
            </h2>

            <p className="text-lg sm:text-xl font-medium text-slate-700 mt-2 bg-zinc-50 p-3 rounded-xl border border-zinc-200">
              💡 <span className="font-semibold">Instructions:</span> {nextMedication.instructions}
            </p>
          </div>
        </div>

        {/* Giant One-Tap Action Buttons */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
          ...
          <button
            ...
            className={`min-h-tap-lg sm:min-h-tap-xl px-6 py-4 rounded-2xl text-xl sm:text-2xl font-bold flex items-center justify-center gap-3 transition-all shadow-md transform active:scale-98 ${
              isTaken
          ...
          <button
            ...
            className={`min-h-tap-lg sm:min-h-tap-xl px-6 py-4 rounded-2xl text-xl sm:text-2xl font-bold flex items-center justify-center gap-3 transition-all border-2 border-zinc-300 bg-zinc-100 hover:bg-zinc-200 text-slate-900 shadow-sm transform active:scale-98 ${
